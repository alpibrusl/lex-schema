...
## Project-specific overrides — lex-schema

- **Schemas are pure.** Nothing under `src/` should declare effects.
  Validation is a `Json -> Result[Json, List[Error]]` shape; if you
  need IO to validate, you're writing the wrong layer.
- **Public surface:** `json_value`, `schema`, `builders`. Don't add to
  the root re-export list without an `examples {}` block on every new
  fn.
- **`ModelSchema` values are `examples {}`-backed.** Every public
  validator gets at least one Ok-case and one Err-case folded into
  its example block; those run at `lex check` time.
- **`required_str` / `optional_*` builder family is the only sanctioned
  way to declare a field.** Don't hand-roll constraint records.
