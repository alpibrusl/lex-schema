...
fn post_schema() -> s.ModelSchema {
  { title: "Post", description: "A blog post backed by a relational row", fields: [s.required_str("id", [StrUuid]), s.required_str("slug", [StrMinLen(3), StrMaxLen(80), StrPattern("^[a-z0-9-]+$")]), s.required_str("title", [StrMinLen(1), StrMaxLen(200)]), s.required_str("body", []), s.required_str("status", [StrOneOf(["draft", "published", "archived"])]), s.required_int("read_minutes", [IntInRange(1, 600)]), s.required_bool("featured"), s.optional(s.required_str("og_image", [StrUrl]))] }
}

# ---- A nested model: Order -> Buyer (FK relationship) -----------
fn order_schema() -> s.ModelSchema {
  { title: "Order", description: "Order with embedded buyer record", fields: [s.required_str("id", [StrUuid]), s.required_str("status", [StrOneOf(["pending", "paid", "shipped", "delivered", "cancelled"])]), s.required_int("total_cents", [IntPositive]), s.required_object("buyer", { title: "Buyer", description: "Person placing the order", fields: [s.required_str("id", [StrUuid]), s.required_str("email", [StrEmail]), s.required_str("phone", [StrPhoneE164])] })] }
}
