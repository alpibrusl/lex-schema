...
fn payment_v1() -> s.ModelSchema {
  { title: "Payment", description: "A card payment (v1)", fields: [s.required_str("cc", [StrCreditCardLuhn]), s.required_int("amount", [IntPositive, IntMax(1000000)]), s.required_str("payer_ip", [StrIPv4])] }
}

# v2 — `cc` renamed to `card_number`, `cvv` required, `phone`
# optional. Same network this version writes to.
fn payment_v2() -> s.ModelSchema {
  { title: "Payment", description: "A card payment (v2)", fields: [s.required_str("card_number", [StrCreditCardLuhn]), s.required_str("cvv", [StrPattern("^[0-9]{3,4}$")]), s.required_int("amount", [IntPositive, IntMax(1000000)]), s.required_str("payer_ip", [StrIPv4]), s.optional(s.required_str("phone", [StrPhoneE164]))] }
}

# ---- Validation pipeline ------------------------------------------
fn parse_and_validate(body :: Str) -> Result[jv.Json, e.Errors] {
  cm.and_then(jv.parse_into_errors(body), fn (j :: jv.Json) -> Result[jv.Json, e.Errors] {
...
fn demo_validate_v2() -> Result[jv.Json, e.Errors] {
  parse_and_validate("{\"card_number\":\"4111111111111111\",\"cvv\":\"123\",\"amount\":2500,\"payer_ip\":\"192.168.1.1\"}")
}

fn demo_validate_bad() -> Result[jv.Json, e.Errors] {
  parse_and_validate("{\"card_number\":\"4111111111111112\",\"cvv\":\"12\",\"amount\":-1,\"payer_ip\":\"256.0.0.1\"}")
}
...
fn demo_problem_json() -> Str {
  match demo_validate_bad() {
    Ok(_) => "(no errors)",
    Err(es) => {
      let p := pr.validation_problem("https://example.com/problems/payment-validation", "/v1/payments", es)
      pr.to_pretty(p)
    },
  }
}
...
# ---- Demos: SDK codegen -------------------------------------------
fn demo_typescript() -> Str {
  sdk.to_typescript(payment_v2())
}

fn demo_zod() -> Str {
  sdk.to_zod(payment_v2())
}

fn demo_rust() -> Str {
  sdk.to_rust_struct(payment_v2())
}

fn demo_python() -> Str {
  sdk.to_python(payment_v2())
}
...
fn v1_to_v2_migration() -> List[m.Transform] {
  [Rename({ from: "cc", to: "card_number" }), AddField({ name: "cvv", default: JStr("000") })]
}
