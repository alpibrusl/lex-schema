...
fn validate(qs :: Map[Str, Str]) -> Result[ListParams, List[e.Error]] {
  cm.combine4(
    ...
    coerce.require_bool_from_map(qs, "include_archived"),
    mk
  )
}
...
fn parse_query(s :: Str) -> Map[Str, Str] {
  # Tiny ad-hoc query-string parser. Real services hand a parsed
  # Map[Str, Str] off `net.parse_query` once that lands; this stays
