...
fn user_in_schema() -> s.ModelSchema {
  {
    title: "UserIn", description: "User signup request",
    fields: [
...
fn user_out_schema() -> s.ModelSchema {
  {
    title: "UserOut", description: "User as returned to clients",
    fields: [
      s.required_int("id",   []),
      s.required_str("name", [StrMinLen(1), StrMaxLen(40)]),
    ],
...
fn user_in() -> v.Validator  { v.make(user_in_schema())  }
fn user_out() -> v.Validator { v.make(user_out_schema()) }
...
fn store_record() -> jv.Json {
  JObj([
    ("id",            JInt(42)),
    ("name",          JStr("alice")),
    ("password_hash", JStr("$2b$12$INTERNAL_DO_NOT_LEAK")),
    ("audit_log",     JStr("created via /signup at 2026-05-13T05:00Z")),
    ("internal_uid",  JStr("u-internal-9387")),
    ("created_at",    JStr("2026-05-13T05:00Z")),
  ])
}
...
fn demo_lossy() -> [io] Nil {
  match v.serialize_lossy(user_out(), store_record()) {
    ...
    Ok(out) => {
      let _ := io.print("== serialize_lossy ==")
      let _ := io.print(out)
      ()
    },
  }
...
fn demo_strict() -> [io] Nil {
  match v.serialize_strict(user_out(), store_record()) {
    ...
    Err(es) => {
      let _ := io.print("== serialize_strict — surfaces internal-field leaks ==")
      let _ := io.print(e.format(es))
      ()
    },
  }
}
...
fn demo_strict_clean() -> [io] Nil {
  ...
  match v.serialize_strict(user_out(), clean) {
    Err(es) => io.print(str.concat("err: ", e.format(es))),
    Ok(out) => {
      let _ := io.print("== serialize_strict (clean payload) ==")
      let _ := io.print(out)
      ()
    },
  }
...
fn demo_openapi() -> [io] Nil {
  ...
  let _ := io.print("== openapi_response (responses[200] body) ==")
  let _ := io.print(jv.stringify_pretty(frag))
  ()
}
...
type HandlerErr =
    BadRequest(e.Errors)       # client error — 400/422
  | InternalContract(e.Errors) # server bug — 500

fn signup(body :: jv.Json, db_record :: jv.Json) -> Result[Str, HandlerErr] {
  match v.validate(user_in(), body) {
    Err(es) => Err(BadRequest(es)),
    Ok(_in) =>
      # business logic would write to the DB here, returning the
      # store record. We pretend it returned `db_record` directly.
      match v.serialize_strict(user_out(), db_record) {
...
fn demo_handler_leak() -> [io] Nil {
  ...
  match signup(req, store_record()) {
    ...
    Err(InternalContract(es)) => {
      let _ := io.print("== handler caught a response-contract drift ==")
      let _ := io.print(e.format(es))
      ()
    },
...
fn demo_handler_clean() -> [io] Nil {
  ...
  match signup(req, clean) {
    Ok(json) => {
      let _ := io.print("== handler (clean store record) ==")
      let _ := io.print(json)
      ()
    },
...
fn main() -> [io] Nil {
  ...
  let _ := demo_strict()
  let _ := io.print("")
  let _ := demo_strict_clean()
  let _ := io.print("")
  let _ := demo_openapi()
  let _ := io.print("")
  let _ := demo_handler_leak()
  let _ := io.print("")
  let _ := demo_handler_clean()
  ()
}
