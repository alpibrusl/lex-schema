...
type ScheduledEvent = { title :: Str, scheduled_for :: Str, organizer_email :: Str, city :: Str }

fn mk_event(t :: Str, s :: Str, em :: Str, ci :: Str) -> ScheduledEvent {
...
fn validate(j :: jv.Json) -> Result[ScheduledEvent, e.Errors] {
  cm.combine4(jv.j_str("", j, "title", [StrMinLen(1), StrMaxLen(120)]), dt_field(j, "scheduled_for", [DateAtOrBefore(upper_bound())]), jv.j_str_at(j, "organizer.email", [StrEmail]), jv.j_str_at(j, "location.address.city", [StrMinLen(1)]), mk_event)
}

# Small adapter: pull a Str field from `j` and run it through
# `dt.check_iso_datetime`. Keeps the call site reading like the
# other combinators.
fn dt_field(j :: jv.Json, field :: Str, checks :: List[dt.DateCheck]) -> Result[Str, e.Errors] {
...
fn parse_event(body :: Str) -> Result[ScheduledEvent, e.Errors] {
  cm.and_then(jv.parse_into_errors(body), fn (j :: jv.Json) -> Result[ScheduledEvent, e.Errors] {
    validate(j)
  })
}
...
fn demo_good() -> Result[ScheduledEvent, e.Errors] {
  parse_event("{\"title\":\"Team offsite\",\"scheduled_for\":\"2026-08-12T09:30:00Z\",\"organizer\":{\"email\":\"alice@example.com\"},\"location\":{\"address\":{\"city\":\"Berlin\"}}}")
}

fn demo_bad() -> Result[ScheduledEvent, e.Errors] {
  parse_event("{\"title\":\"\",\"scheduled_for\":\"2030-06-01T10:00:00Z\",\"organizer\":{\"email\":42},\"location\":{\"address\":{}}}")
}
