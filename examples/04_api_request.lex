...
# ---- Business types -----------------------------------------------
type Signup = { email :: Str, username :: Str, age :: Int }

type RawSignup = { email :: Str, username :: Str, age :: Int }
...
fn validate_signup(raw :: RawSignup) -> Result[Signup, e.Errors] {
  cm.combine3(f.check_str("email", raw.email, [StrEmail]), f.check_str("username", raw.username, [StrMinLen(3), StrMaxLen(32), StrPattern("^[a-zA-Z0-9_]+$")]), f.check_int("age", raw.age, [IntInRange(13, 130)]), mk_signup)
}

fn parse_signup(body :: Str) -> Result[Signup, e.Errors] {
  cm.and_then(p.from_json(body, ["email", "username", "age"]), fn (raw :: RawSignup) -> Result[Signup, e.Errors] {
    validate_signup(raw)
  })
}
  ...
  match req.method {
    "POST" => match req.path {
      "/signup" => match parse_signup(req.body) {
        Ok(s) => { status: 200, body: str.concat("{\"ok\":true,\"username\":\"", str.concat(s.username, "\"}")) },
