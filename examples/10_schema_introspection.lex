...
fn address_schema() -> s.ModelSchema {
  { title: "Address", description: "Postal address", fields: [s.required_str("street", [StrMinLen(1), StrMaxLen(120)]), s.required_str("city", [StrMinLen(1), StrMaxLen(80)]), s.required_str("zip", [StrPattern("^[0-9]{5}$")]), s.required_str("country", [StrPattern("^[A-Z]{2}$")])] }
}

# ---- Top-level schema ---------------------------------------------
fn user_schema() -> s.ModelSchema {
  { title: "User", description: "A registered user with billing address", fields: [s.required_str("name", [StrMinLen(1), StrMaxLen(80)]), s.required_str("email", [StrEmail]), s.required_int("age", [IntInRange(13, 130)]), s.optional(s.required_str("nickname", [StrMaxLen(40)])), s.required_object("address", address_schema()), s.required_array("tags", KStr([StrMinLen(1), StrMaxLen(20)]), [ListMaxLen(10)])] }
}
...
fn demo_validate() -> Result[jv.Json, e.Errors] {
  parse_and_validate("{\"name\":\"Alice\",\"email\":\"alice@example.com\",\"age\":29,\"address\":{\"street\":\"1 Market St\",\"city\":\"SF\",\"zip\":\"94103\",\"country\":\"US\"},\"tags\":[\"vip\",\"beta\"]}")
}

fn demo_validate_bad() -> Result[jv.Json, e.Errors] {
  parse_and_validate("{\"name\":\"Bob\",\"email\":\"nope\",\"age\":7,\"address\":{\"street\":\"x\",\"city\":\"SF\",\"zip\":\"94103\",\"country\":\"us\"},\"tags\":[\"vip\",\"\"]}")
}
