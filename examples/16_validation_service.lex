...
fn validate_request(body :: Str) -> (Int, Str) {
  match jv.parse(body) {
    Err(p) => (400, error_body_many(e.single("", e.code_parse(),
      str.concat(p.message, str.concat(" at byte ", int_to_str(p.pos)))))),
    Ok(envelope) => match jv.get_field(envelope, "schema") {
      None => (400, error_body_many(e.single("schema", e.code_missing(),
        "request must carry a `schema` field"))),
      Some(schema_json) => match si.from_json_schema(schema_json) {
        ...
        Ok(schema) => match jv.get_field(envelope, "payload") {
          None => (400, error_body_many(e.single("payload", e.code_missing(),
            "request must carry a `payload` field"))),
  ...
  match req.method {
    "POST" => match req.path {
      "/v1/validate" => {
        ...
        let status := match result { (s, _) => s }
        let body   := match result { (_, b) => b }
...
fn smoke_good() -> (Int, Str) {
  validate_request(
    "{\"schema\":{\"type\":\"object\",\"properties\":{\"name\":{\"type\":\"string\",\"minLength\":1}},\"required\":[\"name\"]},\"payload\":{\"name\":\"alice\"}}"
  )
}

fn smoke_bad() -> (Int, Str) {
  validate_request(
    "{\"schema\":{\"type\":\"object\",\"properties\":{\"age\":{\"type\":\"integer\",\"minimum\":0}},\"required\":[\"age\"]},\"payload\":{\"age\":-5}}"
  )
}
