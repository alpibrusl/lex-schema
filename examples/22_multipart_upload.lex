...
fn boundary() -> Str { "WebKitFormBoundary7MA4YWxkTrZu0gW" }

fn content_type_header() -> Str {
  str.concat("multipart/form-data; boundary=", boundary())
}

fn bdry()  -> Str { str.concat("--", boundary()) }
fn sep()   -> Str { str.concat(bdry(), "\r\n") }
fn close() -> Str { str.concat(bdry(), "--\r\n") }
...
fn sample_body() -> Str {
  str.join([
    ...
    sep(),
    "Content-Disposition: form-data; name=\"avatar\"; filename=\"selfie.png\"\r\n",
...
fn find_part(
  parts :: List[form.MultipartPart],
  name :: Str
) -> Option[form.MultipartPart] {
  match list.head(parts) {
    None    => None,
    Some(p) => if p.name == name { Some(p) }
               else { find_part(list.tail(parts), name) },
  }
}

fn build_submission(
  uid :: Str, cap :: Str, fname :: Str, ct :: Str, sz :: Int
) -> AvatarSubmission {
  { user_id: uid,
    caption: cap,
    filename: fname,
...
fn validate_avatar(
  parts :: List[form.MultipartPart]
) -> Result[AvatarSubmission, e.Errors] {
  match find_part(parts, "avatar") {
    None    => Err(e.single("avatar", e.code_missing(),
                            "avatar file part missing")),
    Some(p) => match p.filename {
      None      => Err(e.single("avatar", "format",
                                "avatar part must carry a filename")),
      Some(fnm) => {
        let m := form.parts_to_map(parts)
        cm.combine5(
          coerce.require_str_from_map(m, "user_id",
            [StrMinLen(1), StrMaxLen(64),
             StrPattern("^[a-zA-Z0-9_-]+$")]),
          coerce.require_str_from_map(m, "caption", [StrMaxLen(280)]),
          Ok(fnm),
          Ok(p.content_type),
          Ok(str.len(p.body)),
          build_submission
...
fn demo_validate_avatar_bad() -> Result[AvatarSubmission, e.Errors] {
  # Same body shape but with a bogus user_id that breaks the pattern.
  let body := str.join([
    sep(),
    "Content-Disposition: form-data; name=\"user_id\"\r\n\r\n",
    "not a valid id!\r\n",
