...
type WebhookEvent =
  ...
  | Purchase({ user_id :: Str, cents :: Int, sku :: Str })
  | Cancel({ user_id :: Str, reason :: Str })

# ---- Per-variant validators ---------------------------------------
# Each takes the parsed Json and produces a `WebhookEvent` (or an
...
fn validate_signup(j :: jv.Json) -> Result[WebhookEvent, e.Errors] {
  cm.combine2(
    ...
    fn (uid :: Str, em :: Str) -> WebhookEvent {
...
fn validate_purchase(j :: jv.Json) -> Result[WebhookEvent, e.Errors] {
  cm.combine3(
    ...
    jv.j_str("", j, "sku",     [StrPattern("^[A-Z]{3}-[0-9]{4}$")]),
    fn (uid :: Str, c :: Int, s :: Str) -> WebhookEvent {
