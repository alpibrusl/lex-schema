...
type User = { email :: Str, username :: Str, age :: Int, password :: Str }
...
type RawUser = { email :: Str, username :: Str, age :: Int, password :: Str }

fn validate(raw :: RawUser) -> Result[User, e.Errors] {
  cm.combine4(f.check_str("email", raw.email, [StrEmail]), f.check_str("username", raw.username, [StrMinLen(3), StrMaxLen(32), StrPattern("^[a-zA-Z0-9_]+$")]), f.check_int("age", raw.age, [IntInRange(13, 130)]), f.check_str("password", raw.password, [StrMinLen(8), StrMaxLen(128), StrPattern(".*[0-9].*")]), build_user)
}

# ---- End-to-end: parse JSON, then validate ------------------------
fn parse_and_validate(input :: Str) -> Result[User, e.Errors] {
  cm.and_then(p.from_json(input, ["email", "username", "age", "password"]), fn (raw :: RawUser) -> Result[User, e.Errors] {
    validate(raw)
  })
}
...
fn validate_good() -> Result[User, e.Errors] {
  parse_and_validate("{\"email\":\"alice@example.com\",\"username\":\"alice_42\",\"age\":29,\"password\":\"correcthorse9\"}")
}

fn validate_bad() -> Result[User, e.Errors] {
  parse_and_validate("{\"email\":\"not-an-email\",\"username\":\"a!\",\"age\":7,\"password\":\"weak\"}")
}
