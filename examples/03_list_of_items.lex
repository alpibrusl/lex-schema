...
# ---- Types --------------------------------------------------------
type LineItem = { sku :: Str, quantity :: Int, price_cents :: Int }

type Order = { customer :: Str, items :: List[LineItem] }

type RawItem = { sku :: Str, quantity :: Int, price_cents :: Int }

type RawOrder = { customer :: Str, items :: List[RawItem] }
...
fn validate_item(path :: Str, raw :: RawItem) -> Result[LineItem, e.Errors] {
  cm.with_path(path, cm.combine3(f.check_str("sku", raw.sku, [StrPattern(sku_pattern())]), f.check_int("quantity", raw.quantity, [IntPositive, IntMax(1000)]), f.check_int("price_cents", raw.price_cents, [IntMin(0), IntMax(1000000)]), mk_item))
}
...
fn validate_order(raw :: RawOrder) -> Result[Order, e.Errors] {
  cm.combine2(f.check_str("customer", raw.customer, [StrMinLen(1), StrMaxLen(80)]), validate_items(raw.items), mk_order)
}

fn parse_order(input :: Str) -> Result[Order, e.Errors] {
  cm.and_then(p.from_json(input, ["customer", "items"]), fn (raw :: RawOrder) -> Result[Order, e.Errors] {
    validate_order(raw)
  })
}
...
fn validate_good() -> Result[Order, e.Errors] {
  parse_order("{\"customer\":\"Acme\",\"items\":[{\"sku\":\"ABC-1234\",\"quantity\":2,\"price_cents\":2500},{\"sku\":\"XYZ-7777\",\"quantity\":1,\"price_cents\":499}]}")
}

fn validate_bad() -> Result[Order, e.Errors] {
  parse_order("{\"customer\":\"\",\"items\":[{\"sku\":\"abc\",\"quantity\":1,\"price_cents\":100},{\"sku\":\"DEF-1111\",\"quantity\":-1,\"price_cents\":-50},{\"sku\":\"XX\",\"quantity\":0,\"price_cents\":10}]}")
}
