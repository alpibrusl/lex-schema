...
fn build_spec() -> Json {
  cli.spec("genctl", "generate things", [
    cli.flag("verbose", Some("v"), "noisy output"),
    cli.option("output", Some("o"), "output file path", Some("out.txt")),
    cli.positional("path", "input directory", true),
  ], [])
}
...
fn arg_schema() -> s.ModelSchema {
  {
    title: "GenCtlArgs", description: "",
    fields: [
      # Positional `path` must be a non-empty string.
      s.required_str("path", [StrMinLen(1), StrMaxLen(1024)]),
      # `--output` defaults to "out.txt" via the cli spec; we
      # additionally require a `.txt` suffix.
      s.required_str("output", [StrEndsWith(".txt"), StrMaxLen(255)]),
      # `--verbose` is a flag (Bool).
      s.required_bool("verbose"),
    ],
  }
}
...
fn show_help() -> Str { cl.help(build_spec()) }
