...
fn original() -> s.ModelSchema {
  { title: "User", description: "", fields: [s.required_str("name", [StrMinLen(1), StrMaxLen(80)]), s.required_str("email", [StrEmail]), s.required_int("age", [IntInRange(13, 130)])] }
}
...
fn demo_roundtrip() -> Result[Str, e.Errors] {
  ...
  match si.from_str(s_text) {
    Err(es) => Err(es),
    Ok(imported) => match jv.parse_into_errors("{\"name\":\"Alice\",\"email\":\"alice@example.com\",\"age\":29}") {
      Err(es) => Err(es),
...
fn demo_external_schema() -> Result[Str, e.Errors] {
  ...
  match si.from_str(external_schema_text) {
    Err(es) => Err(es),
    Ok(schema) => match jv.parse_into_errors("{\"sku\":\"ABC-1234\",\"price\":2500}") {
      Err(es) => Err(es),
