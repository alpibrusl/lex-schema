...
# ---- Types --------------------------------------------------------
type Address = { street :: Str, city :: Str, zip :: Str, country :: Str }

type User = { name :: Str, email :: Str, address :: Address }

type RawAddress = { street :: Str, city :: Str, zip :: Str, country :: Str }

type RawUser = { name :: Str, email :: Str, address :: RawAddress }
...
fn validate_address(raw :: RawAddress) -> Result[Address, e.Errors] {
  cm.combine4(f.check_str("street", raw.street, [StrMinLen(1), StrMaxLen(120)]), f.check_str("city", raw.city, [StrMinLen(1), StrMaxLen(80)]), f.check_str("zip", raw.zip, [StrPattern(zip_pattern())]), f.check_str("country", raw.country, [StrPattern(country_pattern())]), mk_address)
}

# ---- Outer validator ----------------------------------------------
fn validate_user(raw :: RawUser) -> Result[User, e.Errors] {
  cm.combine3(f.check_str("name", raw.name, [StrMinLen(1), StrMaxLen(80)]), f.check_str("email", raw.email, [StrEmail]), cm.with_path("address", validate_address(raw.address)), mk_user)
}

# ---- End-to-end ---------------------------------------------------
fn parse_user(input :: Str) -> Result[User, e.Errors] {
  cm.and_then(p.from_json(input, ["name", "email", "address"]), fn (raw :: RawUser) -> Result[User, e.Errors] {
    validate_user(raw)
  })
}
...
fn validate_good() -> Result[User, e.Errors] {
  parse_user("{\"name\":\"Alice\",\"email\":\"alice@example.com\",\"address\":{\"street\":\"1 Market St\",\"city\":\"SF\",\"zip\":\"94103\",\"country\":\"US\"}}")
}

fn validate_bad() -> Result[User, e.Errors] {
  parse_user("{\"name\":\"Bob\",\"email\":\"nope\",\"address\":{\"street\":\"\",\"city\":\"\",\"zip\":\"abc\",\"country\":\"USA\"}}")
}
