...
fn str_len_bounds(checks :: List[c.StrCheck]) -> (Int, Int) {
  list.fold(checks, (1, 32),
    fn (acc :: (Int, Int), chk :: c.StrCheck) -> (Int, Int) {
      let lo := match acc { (l, _) => l }
      let hi := match acc { (_, h) => h }
      match chk {
...
fn int_bounds(checks :: List[c.IntCheck]) -> (Int, Int) {
  list.fold(checks, default_int_bounds(),
    fn (acc :: (Int, Int), chk :: c.IntCheck) -> (Int, Int) {
      let lo := match acc { (l, _) => l }
      let hi := match acc { (_, h) => h }
      match chk {
...
fn default_int_bounds() -> (Int, Int) { (0 - 1000000, 1000000) }

fn float_bounds(checks :: List[c.FloatCheck]) -> (Float, Float) {
  list.fold(checks, (0.0 - 1000.0, 1000.0),
    fn (acc :: (Float, Float), chk :: c.FloatCheck) -> (Float, Float) {
      let lo := match acc { (l, _) => l }
      let hi := match acc { (_, h) => h }
      match chk {
...
fn list_len_bounds(checks :: List[c.ListCheck]) -> (Int, Int) {
  list.fold(checks, (0, 5),
    fn (acc :: (Int, Int), chk :: c.ListCheck) -> (Int, Int) {
      let lo := match acc { (l, _) => l }
      let hi := match acc { (_, h) => h }
      match chk {
...
fn generate(schema :: s.ModelSchema, rng :: Rng) -> (jv.Json, Rng) {
  let result := list.fold(schema.fields, init_obj(rng),
    fn (acc :: (List[(Str, jv.Json)], Rng), field :: s.Field)
      -> (List[(Str, jv.Json)], Rng) {
      let entries := match acc { (e1, _r) => e1 }
      let r       := match acc { (_e, r1) => r1 }
      if field.required {
        let gen := gen_kind(field.kind, r)
        let v   := match gen { (v1, _) => v1 }
        let r2  := match gen { (_, r1) => r1 }
        (list.concat(entries, [(field.name, v)]), r2)
      } else {
        # 50% chance to emit; otherwise skip the field.
        let coin := random.int(r, 0, 1)
        let bit  := match coin { (b, _)  => b }
        let r1   := match coin { (_, r1) => r1 }
        if bit == 1 {
          let gen := gen_kind(field.kind, r1)
          let v   := match gen { (v1, _) => v1 }
          let r2  := match gen { (_, r2) => r2 }
          (list.concat(entries, [(field.name, v)]), r2)
        } else {
          (entries, r1)
        }
  ...
  let entries := match result { (e1, _) => e1 }
  let rng_out := match result { (_, r1) => r1 }
  (JObj(entries), rng_out)
}

# Polymorphic empty accumulator (lex-lang#319 workaround).
fn init_obj(rng :: Rng) -> (List[(Str, jv.Json)], Rng) { ([], rng) }
fn init_list(rng :: Rng) -> (List[jv.Json], Rng) { ([], rng) }
...
fn gen_str(checks :: List[c.StrCheck], rng :: Rng) -> (jv.Json, Rng) {
  if has_email(checks)         { gen_email(rng) }
  else { if has_url(checks)    { gen_url(rng) }
  else { if has_uuid(checks)   { gen_uuid(rng) }
  else { if has_one_of(checks) { gen_one_of_str(checks, rng) }
  else {
    let bounds := str_len_bounds(checks)
    let lo := match bounds { (l, _) => l }
    let hi := match bounds { (_, h) => h }
    let len_draw := random.int(rng, lo, hi)
    let len_ := match len_draw { (n, _) => n }
    let r := match len_draw { (_, r1) => r1 }
...
fn gen_one_of_str(checks :: List[c.StrCheck], rng :: Rng) -> (jv.Json, Rng) {
  # Take the first `StrOneOf`; subsequent ones would contradict.
  let opts := find_one_of(checks)
  match random.choose(rng, opts) {
    None    => (JStr(""), rng),
    Some(p) => match p { (s, r) => (JStr(s), r) },
  }
}
...
fn gen_str_of_len(len_ :: Int, rng :: Rng) -> (jv.Json, Rng) {
  ...
  let result := list.fold(list.range(0, len_), gen_str_init(rng),
    fn (acc :: (Str, Rng), _i :: Int) -> (Str, Rng) {
      let s := match acc { (s1, _) => s1 }
      let r := match acc { (_, r1) => r1 }
      match random.choose(r, alphabet) {
        None       => (s, r),
        Some(pair) => match pair { (c, r2) => (str.concat(s, c), r2) },
      }
    })
  let s := match result { (s1, _) => s1 }
  let r := match result { (_, r1) => r1 }
  (JStr(s), r)
}

fn gen_str_init(rng :: Rng) -> (Str, Rng) { ("", rng) }
...
fn gen_email(rng :: Rng) -> (jv.Json, Rng) {
  ...
  let l := match local { (JStr(s), _) => s, _ => "" }
  let r := match local { (_, r1) => r1 }
  (JStr(str.concat(l, "@example.com")), r)
}

fn gen_url(rng :: Rng) -> (jv.Json, Rng) {
  let host := gen_str_of_len(8, rng)
  let h := match host { (JStr(s), _) => s, _ => "" }
  let r := match host { (_, r1) => r1 }
...
fn gen_uuid(rng :: Rng) -> (jv.Json, Rng) {
  ...
  let s1 := match g8 { (s1, _) => s1 }
  let r1 := match g8 { (_, r1) => r1 }
  let g4a := gen_hex_run(4, r1, hex)
  let s2 := match g4a { (s, _) => s }
  let r2 := match g4a { (_, r) => r }
  let g4b := gen_hex_run(4, r2, hex)
  let s3 := match g4b { (s, _) => s }
  let r3 := match g4b { (_, r) => r }
  let g4c := gen_hex_run(4, r3, hex)
  let s4 := match g4c { (s, _) => s }
  let r4 := match g4c { (_, r) => r }
  let g12 := gen_hex_run(12, r4, hex)
  let s5 := match g12 { (s, _) => s }
  let r5 := match g12 { (_, r) => r }
  let joined := str.concat(s1, str.concat("-",
    str.concat(s2, str.concat("-",
    str.concat(s3, str.concat("-",
    str.concat(s4, str.concat("-", s5))))))))
  (JStr(joined), r5)
}
...
fn gen_int(checks :: List[c.IntCheck], rng :: Rng) -> (jv.Json, Rng) {
  # `IntOneOf` wins if present.
  let opts := find_one_of_int(checks)
  if list.is_empty(opts) {
    let bounds := int_bounds(checks)
    let lo := match bounds { (l, _) => l }
    let hi := match bounds { (_, h) => h }
    let real_hi := if hi < lo { lo } else { hi }
    let draw := random.int(rng, lo, real_hi)
    let n := match draw { (n1, _) => n1 }
    let r := match draw { (_, r1) => r1 }
    (JInt(n), r)
  } else {
    match random.choose(rng, opts) {
      None    => (JInt(0), rng),
      Some(p) => match p { (n, r) => (JInt(n), r) },
    }
  }
}
...
fn gen_array(
  ...
  let hi := match bounds { (_, h) => h }
  let len_draw := random.int(rng, lo, hi)
  let n := match len_draw { (m, _) => m }
  let r := match len_draw { (_, r1) => r1 }
  let result := list.fold(list.range(0, n), init_list(r),
    fn (acc :: (List[jv.Json], Rng), _i :: Int) -> (List[jv.Json], Rng) {
      let items := match acc { (i1, _) => i1 }
      let r1    := match acc { (_, r1) => r1 }
      let gen := gen_kind(elem, r1)
      let v   := match gen { (v1, _) => v1 }
      let r2  := match gen { (_, r2) => r2 }
      (list.concat(items, [v]), r2)
    })
  let items := match result { (i1, _) => i1 }
  let r2    := match result { (_, r1) => r1 }
  (JList(items), r2)
}
...
fn round_trip_loop(
  ...
  if remaining <= 0 { Ok(passed) }
    ...
    let sample := match g { (v, _) => v }
    let r2     := match g { (_, r) => r }
    match s.validate(schema, sample) {
      ...
      Err(es) => {
        ...
        Err(list.concat(
          e.single(path, "round_trip_fail",
            str.concat("generator produced an invalid sample; serialized: ",
              jv.stringify(sample))),
          es))
      },
    }
...
fn max_float(a :: Float, b :: Float) -> Float { if a > b { a } else { b } }
fn min_float(a :: Float, b :: Float) -> Float { if a < b { a } else { b } }
