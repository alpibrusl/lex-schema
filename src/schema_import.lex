...
# ---- Recursive walker --------------------------------------------
fn parse_model_at(path :: Str, j :: jv.Json) -> Result[s.ModelSchema, e.Errors] {
  ...
  match jv.get_field(j, "properties") {
    ...
    Some(p) => match jv.as_obj(p) {
      None => Err(e.single(join_path(path, "properties"), e.code_type(), "expected object")),
      Some(entries) => {
        let walked := list.fold(entries, walk_init(), fn (acc :: (List[s.Field], e.Errors), pair :: (Str, jv.Json)) -> (List[s.Field], e.Errors) {
          let fields := match acc {
            (f, _) => f,
          }
          let errs := match acc {
            (_, e) => e,
          ...
          match parse_field(join_path(path, name), name, v, contains_str(required_set, name)) {
            Ok(field) => (list.concat(fields, [field]), errs),
            Err(es) => (fields, list.concat(errs, es)),
          }
        })
        let fields := match walked {
          (f, _) => f,
        }
...
fn parse_field(path :: Str, name :: Str, j :: jv.Json, required :: Bool) -> Result[s.Field, e.Errors] {
  let desc := str_field_or(j, "description", "")
  match parse_kind(path, j) {
    Err(es) => Err(es),
    Ok(kind) => Ok({ name: name, required: required, description: desc, kind: kind }),
  }
}
...
fn parse_kind(path :: Str, j :: jv.Json) -> Result[s.FieldKind, e.Errors] {
  match jv.get_field(j, "type") {
    ...
    Some(t) => match jv.as_str(t) {
      None => Err(e.single(join_path(path, "type"), e.code_type(), "expected string for `type`")),
      Some(name) => match name {
        ...
        other => Err(e.single(join_path(path, "type"), e.code_one_of(), str.concat("unknown type: ", other))),
      },
    },
  }
}

fn parse_array_kind(path :: Str, j :: jv.Json) -> Result[s.FieldKind, e.Errors] {
...
fn parse_str_checks(j :: jv.Json) -> List[c.StrCheck] {
  ...
  list.concat(list.concat(list.concat(list.concat(from_min, from_max), from_pattern), from_format), from_enum)
}
...
fn parse_int_checks(j :: jv.Json) -> List[c.IntCheck] {
  ...
  list.concat(list.concat(list.concat(list.concat(from_min, from_max), from_excl), from_const), from_enum)
}
