...
# Parse a bound to an Instant. Bad bounds (string didn't parse)
# return None; the per-check evaluator treats that as a fail and
# surfaces the original string in the error message.
...
fn eval_date(c :: DateCheck, ts :: Instant) -> Option[Str] {
  match c {
    DateBefore(hi) => match bound_instant(hi) {
      ...
      Some(h_in) => if datetime.before(ts, h_in) { None }
                    else { Some(str.concat("must be before ", hi)) },
    },
    DateAtOrBefore(hi) => match bound_instant(hi) {
      None       => Some(str.concat("invalid bound: ", hi)),
      Some(h_in) => if datetime.compare(ts, h_in) <= 0 { None }
                    else { Some(str.concat("must be at or before ", hi)) },
    },
    DateAfter(lo) => match bound_instant(lo) {
      None       => Some(str.concat("invalid bound: ", lo)),
      Some(l_in) => if datetime.after(ts, l_in) { None }
                    else { Some(str.concat("must be after ", lo)) },
    },
    DateAtOrAfter(lo) => match bound_instant(lo) {
      None       => Some(str.concat("invalid bound: ", lo)),
      Some(l_in) => if datetime.compare(ts, l_in) >= 0 { None }
                    else { Some(str.concat("must be at or after ", lo)) },
    },
