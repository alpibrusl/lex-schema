...
type Validator = { schema :: s.ModelSchema, json_schema :: jv.Json, openapi :: jv.Json, openapi_response :: jv.Json, typescript :: Str, python :: Str }
...
fn make(schema :: s.ModelSchema) -> Validator {
  ...
  { schema: schema, json_schema: s.to_json_schema(schema), openapi: openapi, openapi_response: build_openapi_response(openapi), typescript: sdk.to_typescript(schema), python: sdk.to_python(schema) }
}
...
fn serialize_strict(v :: Validator, value :: jv.Json) -> Result[Str, e.Errors] {
  let field_names := list.map(v.schema.fields, fn (field :: s.Field) -> Str {
    field.name
  })
  let extra_errs := match jv.as_obj(value) {
    None => [],
    Some(entries) => list.fold(entries, [], fn (acc :: e.Errors, pair :: (Str, jv.Json)) -> e.Errors {
      match pair {
        (key, _) => if is_known_field(field_names, key) {
          acc
        } else {
          list.concat(acc, e.single(key, e.code_extra(), str_concat("unexpected extra field: ", key)))
        },
      }
    }),
  }
...
fn export_json_schema_str(v :: Validator) -> Str {
  jv.stringify_pretty(v.json_schema)
}

fn export_openapi_str(v :: Validator) -> Str {
  jv.stringify_pretty(v.openapi)
}

fn export_typescript(v :: Validator) -> Str {
  v.typescript
}

fn export_python(v :: Validator) -> Str {
  v.python
}
...
fn build_openapi_response(openapi_schema :: jv.Json) -> jv.Json {
  JObj([("200", JObj([("description", JStr("Successful response")), ("content", JObj([("application/json", JObj([("schema", openapi_schema)]))]))]))])
}
...
{
  list.fold(names, false, fn (acc :: Bool, name :: Str) -> Bool {
    acc or name == key
  })
}
