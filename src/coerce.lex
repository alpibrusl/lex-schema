...
fn coerce_str_to_int(path :: Str, s :: Str) -> Result[Int, e.Errors] {
  match str.to_int(str.trim(s)) {
    Some(n) => Ok(n),
    None => Err(e.single(path, e.code_type(), str.concat("expected integer, got ", quote(s)))),
  }
}

fn check_str_as_int(path :: Str, s :: Str, checks :: List[c.IntCheck]) -> Result[Int, e.Errors] {
...
fn coerce_str_to_float(path :: Str, s :: Str) -> Result[Float, e.Errors] {
  match str.to_float(str.trim(s)) {
    Some(x) => Ok(x),
    None => Err(e.single(path, e.code_type(), str.concat("expected number, got ", quote(s)))),
  }
}

fn check_str_as_float(path :: Str, s :: Str, checks :: List[c.FloatCheck]) -> Result[Float, e.Errors] {
...
fn coerce_str_to_bool(path :: Str, s :: Str) -> Result[Bool, e.Errors] {
  ...
  if is_truthy_word(normalized) {
    ...
    if is_falsy_word(normalized) {
      ...
      Err(e.single(path, e.code_type(), str.concat("expected boolean, got ", quote(s))))
    }
  }
}
...
fn require_int_from_map(src :: Map[Str, Str], key :: Str, checks :: List[c.IntCheck]) -> Result[Int, e.Errors] {
...
fn require_float_from_map(src :: Map[Str, Str], key :: Str, checks :: List[c.FloatCheck]) -> Result[Float, e.Errors] {
  match map.get(src, key) {
    None => Err(e.single(key, e.code_missing(), "field is required")),
...
fn require_bool_from_map(src :: Map[Str, Str], key :: Str) -> Result[Bool, e.Errors] {
  match map.get(src, key) {
    None => Err(e.single(key, e.code_missing(), "field is required")),
...
fn require_str_from_map(src :: Map[Str, Str], key :: Str, checks :: List[c.StrCheck]) -> Result[Str, e.Errors] {
  match map.get(src, key) {
    None => Err(e.single(key, e.code_missing(), "field is required")),
...
fn optional_int_from_map(src :: Map[Str, Str], key :: Str, checks :: List[c.IntCheck]) -> Result[Option[Int], e.Errors] {
  match map.get(src, key) {
    None => Ok(None),
...
fn optional_str_from_map(src :: Map[Str, Str], key :: Str, checks :: List[c.StrCheck]) -> Result[Option[Str], e.Errors] {
  match map.get(src, key) {
    None => Ok(None),
