...
fn discriminate[T](path_prefix :: Str, j :: jv.Json, field :: Str, branches :: List[(Str, (jv.Json) -> Result[T, e.Errors])]) -> Result[T, e.Errors] {
  match jv.j_str(path_prefix, j, field, []) {
    Err(es) => Err(es),
    Ok(tag) => match find_branch(branches, tag) {
      Some(handler) => handler(j),
      None => Err(e.single(joined_path(path_prefix, field), e.code_one_of(), str.concat("unknown discriminator `", str.concat(tag, str.concat("`, expected one of ", str.join(branch_tags(branches), ", ")))))),
    },
  }
}

# Lookup helpers — pure list folds.
fn find_branch[T](branches :: List[(Str, (jv.Json) -> Result[T, e.Errors])], tag :: Str) -> Option[(jv.Json) -> Result[T, e.Errors]] {
  list.fold(branches, find_init(), fn (acc :: Option[(jv.Json) -> Result[T, e.Errors]], pair :: (Str, (jv.Json) -> Result[T, e.Errors])) -> Option[(jv.Json) -> Result[T, e.Errors]] {
    match acc {
      Some(_) => acc,
      None => match pair {
        (k, v) => if k == tag {
...
fn branch_tags[T](branches :: List[(Str, (jv.Json) -> Result[T, e.Errors])]) -> List[Str] {
  list.map(branches, fn (pair :: (Str, (jv.Json) -> Result[T, e.Errors])) -> Str {
    match pair {
      (k, _v) => k,
    }
  })
