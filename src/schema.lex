...
fn optional(field :: Field) -> Field
  examples {
    optional(required_str("x", [])) => { name: "x", required: false, description: "", kind: KStr([]) }
  }
{
...
fn validate_array(path :: Str, elem :: FieldKind, xs :: List[jv.Json], shape :: List[c.ListCheck]) -> FieldOutcome {
  ...
  if e.is_ok(all) {
    FieldOk(JList(list.reverse(items)))
  } else {
    FieldErr(all)
  }
}
...
fn to_json_schema(schema :: ModelSchema) -> jv.Json {
  ...
  let final_entries := if str.is_empty(schema.description) {
    entries
  } else {
...
fn schema_body(schema :: ModelSchema) -> List[(Str, jv.Json)] {
  ...
  let required_names := list.reverse(list.fold(schema.fields, [], fn (acc :: List[jv.Json], field :: Field) -> List[jv.Json] {
    if field.required {
      list.cons(JStr(field.name), acc)
    } else {
      acc
    }
  }))
