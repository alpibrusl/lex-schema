...
type Problem = {
  type_uri :: Str,    # absolute URL identifying the problem type
  title :: Str,       # short human-readable summary
  status :: Int,      # HTTP status code
  detail :: Str,      # longer human-readable description
  instance :: Str,    # request path / URI that triggered this
  errors :: e.Errors, # the per-field error list
}
...
fn validation_problem(
...
) -> Problem {
  {
    ...
    title:    "Validation failed",
    status:   422,
    detail:   detail_from_errors(errs),
    instance: instance,
    errors:   errs,
  }
}
...
fn detail_from_errors(errs :: e.Errors) -> Str {
  let n := list.len(errs)
  if n == 1 { "1 field failed validation" }
  else { str.concat(int.to_str(n), " fields failed validation") }
}
...
fn not_found(type_uri :: Str, instance :: Str, detail :: Str) -> Problem {
  { type_uri: type_uri, title: "Not Found", status: 404,
    detail: detail, instance: instance, errors: [] }
}

fn method_not_allowed(type_uri :: Str, instance :: Str, allowed :: Str) -> Problem {
  { type_uri: type_uri, title: "Method Not Allowed", status: 405,
    detail: str.concat("Allowed methods: ", allowed),
    instance: instance, errors: [] }
}

fn unauthorized(type_uri :: Str, instance :: Str, detail :: Str) -> Problem {
  { type_uri: type_uri, title: "Unauthorized", status: 401,
    detail: detail, instance: instance, errors: [] }
}

fn forbidden(type_uri :: Str, instance :: Str, detail :: Str) -> Problem {
  { type_uri: type_uri, title: "Forbidden", status: 403,
    detail: detail, instance: instance, errors: [] }
}

fn bad_request(type_uri :: Str, instance :: Str, detail :: Str) -> Problem {
  { type_uri: type_uri, title: "Bad Request", status: 400,
    detail: detail, instance: instance, errors: [] }
}

fn internal_server_error(type_uri :: Str, instance :: Str, detail :: Str) -> Problem {
  { type_uri: type_uri, title: "Internal Server Error", status: 500,
    detail: detail, instance: instance, errors: [] }
}
...
fn to_json(p :: Problem) -> jv.Json {
  let errs_arr := list.map(p.errors, fn (er :: e.Error) -> jv.Json {
    JObj([
      ("path",    JStr(er.path)),
      ("code",    JStr(er.code)),
      ("message", JStr(er.message)),
    ])
  })
  JObj([
    ("type",     JStr(p.type_uri)),
    ("title",    JStr(p.title)),
    ("status",   JInt(p.status)),
    ("detail",   JStr(p.detail)),
    ("instance", JStr(p.instance)),
    ("errors",   JList(errs_arr)),
  ])
}

# Render as a compact JSON string. The byte string an HTTP handler
# writes as the response body.
fn to_str(p :: Problem) -> Str { jv.stringify(to_json(p)) }

# Pretty-printed JSON. Useful for logging; not what you'd send on
# the wire.
fn to_pretty(p :: Problem) -> Str { jv.stringify_pretty(to_json(p)) }

# Convenience for HTTP handlers: returns a (status, body) tuple
# ready to drop into a Response record.
fn to_response(p :: Problem) -> (Int, Str) { (p.status, to_str(p)) }

# Content-Type the body should be served with.
fn content_type() -> Str { "application/problem+json" }
