...
fn empty_body() -> Result[Unit, Str] {
  ...
  if map.size(m) == 0 { Ok(()) } else { Err("expected empty") }
}
...
fn pct_decodes() -> Result[Unit, Str] {
  ...
  match map.get(m, "city") {
    Some(s) => if str.contains(s, " ") and str.contains(s, "P") {
      Ok(())
    } else { Err(str.concat("decoded form: ", s)) },
    _ => Err("city missing"),
  }
}

fn equals_in_value() -> Result[Unit, Str] {
  # Only the first `=` splits; subsequent ones land in the value.
...
fn dispatch_urlencoded_ok() -> Result[Unit, Str] {
  match form.decode_body("a=1", "application/x-www-form-urlencoded") {
    Ok(m)  => if map.size(m) == 1 { Ok(()) } else { Err("wrong size") },
    Err(_) => Err("rejected"),
  }
}

fn dispatch_with_charset_suffix() -> Result[Unit, Str] {
  # Real browsers send "application/x-www-form-urlencoded; charset=UTF-8"
  # — must accept that as the same content-type.
  match form.decode_body("a=1",
