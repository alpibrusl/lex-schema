...
fn prefix_path_handles_empty_inner() -> Result[Unit, Str] {
  ...
  match list.head(prefixed) {
    None     => Err("empty"),
    Some(er) => if er.path == "address" { Ok(()) } else {
...
fn prefix_index_emits_brackets() -> Result[Unit, Str] {
  ...
  match list.head(prefixed) {
    None     => Err("empty"),
    Some(er) => if er.path == "items[3].name" { Ok(()) } else {
      Err(str.concat("wrong path: ", er.path))
    },
  }
}
...
fn format_root_path() -> Result[Unit, Str] {
  ...
  if e.format(es) == "<root>: bad json [parse]" { Ok(()) } else {
    Err(str.concat("got: ", e.format(es)))
  }
}

fn is_ok_empty_true() -> Result[Unit, Str] {
  if e.is_ok([]) { Ok(()) } else { Err("[] should be ok") }
}
...
fn suite() -> List[Result[Unit, Str]] {
  [
    error_record_shape(),
    single_makes_one_item_list(),
    concat_merges(),
    flatten_handles_empty(),
    prefix_path_handles_empty_inner(),
    prefix_path_joins_with_dot(),
    prefix_index_emits_brackets(),
    format_one(),
    format_root_path(),
    is_ok_empty_true(),
    is_ok_nonempty_false(),
  ]
}
