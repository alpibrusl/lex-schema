...
fn fixture_spec() -> Json {
  cli.spec("t", "test cli", [
    cli.flag("debug", Some("d"), ""),
    cli.option("output", Some("o"), "", Some("default.txt")),
    cli.positional("input", "", true),
  ], [])
}
...
fn fixture_schema() -> s.ModelSchema {
  {
    ...
    fields: [
      s.required_str("input",  [StrMinLen(1)]),
      s.required_str("output", [StrEndsWith(".txt")]),
      s.required_bool("debug"),
    ],
  }
}
...
fn happy_path() -> Result[Unit, Str] {
  match cl.parse_and_validate_argv(fixture_spec(), ["./x", "--debug"],
                                    fixture_schema()) {
    Ok(_)  => Ok(()),
...
fn option_default_flows_through() -> Result[Unit, Str] {
  match cl.parse_and_validate_argv(fixture_spec(), ["./x"], fixture_schema()) {
    ...
    Ok(j)  => match jv.j_str("", j, "output", []) {
      Ok(s) => if s == "default.txt" { Ok(()) }
               else { Err(str.concat("wrong default: ", s)) },
...
fn cli_parse_error_tagged() -> Result[Unit, Str] {
  # Missing the required positional `input`.
...
fn schema_constraint_runs() -> Result[Unit, Str] {
  # Spec accepts any string for `--output`; schema requires `.txt`.
  match cl.parse_and_validate_argv(fixture_spec(),
                                    ["./x", "--output", "report.pdf"],
                                    fixture_schema()) {
    Ok(_)   => Err("expected Err"),
    Err(es) => match list.head(es) {
      None     => Err("empty"),
      Some(er) => if er.path == "output" { Ok(()) } else {
...
fn flatten_merges_buckets() -> Result[Unit, Str] {
  let parsed := JObj([
    ("positionals", JObj([("input", JStr("p"))])),
    ("flags",       JObj([("debug", JBool(true))])),
    ("options",     JObj([("output", JStr("o.txt"))])),
    ("command",     JList([JStr("t")])),
  ])
...
fn help_passthrough() -> Result[Unit, Str] {
  ...
  if str.contains(h, "test cli") { Ok(()) } else {
...
fn suite() -> List[Result[Unit, Str]] {
  [
    happy_path(),
    option_default_flows_through(),
    cli_parse_error_tagged(),
    schema_constraint_runs(),
    flatten_merges_buckets(),
    help_passthrough(),
  ]
}
