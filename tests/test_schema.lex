  ...
  match jv.parse("{\"name\":\"Alice\"}") {
    ...
    Ok(j)  => match s.validate(user_schema(), j) {
      ...
      Err(es) => match list.head(es) {
        None     => Err("empty"),
        Some(er) => if er.code == "missing" and er.path == "age" { Ok(()) }
                    else { Err(str.concat("got: ", er.path)) },
      },
    },
  }
}
...
fn validate_type_mismatch() -> Result[Unit, Str] {
  match jv.parse("{\"name\":\"Alice\",\"age\":\"thirty\"}") {
    ...
    Ok(j)  => match s.validate(user_schema(), j) {
      ...
      Err(es) => match list.head(es) {
        None     => Err("empty"),
        Some(er) => if er.code == "type" and er.path == "age" { Ok(()) }
                    else { Err(str.concat("got code/path: ", er.code)) },
      },
    },
  }
}
...
fn validate_optional_absent_ok() -> Result[Unit, Str] {
  # `nickname` is optional; absent input is fine.
  match jv.parse("{\"name\":\"Alice\",\"age\":30}") {
    Err(_) => Err("parse"),
    Ok(j)  => match s.validate(user_schema(), j) {
      Ok(_)  => Ok(()),
...
fn nested_schema() -> s.ModelSchema {
  {
    ...
    fields: [
      s.required_object("inner", {
        title: "Inner", description: "",
        fields: [s.required_int("n", [IntPositive])],
      }),
    ],
  }
}
...
fn nested_path_in_error() -> Result[Unit, Str] {
  match jv.parse("{\"inner\":{\"n\":-1}}") {
    ...
    Ok(j)  => match s.validate(nested_schema(), j) {
      ...
      Err(es) => match list.head(es) {
        None     => Err("empty"),
        Some(er) => if er.path == "inner.n" { Ok(()) } else {
...
fn array_schema() -> s.ModelSchema {
  {
    ...
    fields: [s.required_array("items", KStr([StrMinLen(1)]), [ListMinLen(1)])],
  }
}
...
fn array_element_error_indexed() -> Result[Unit, Str] {
  match jv.parse("{\"items\":[\"ok\",\"\",\"also\"]}") {
    ...
    Ok(j)  => match s.validate(array_schema(), j) {
      ...
      Err(es) => match list.head(es) {
        None     => Err("empty"),
        Some(er) => if er.path == "items[1]" { Ok(()) } else {
          Err(str.concat("wrong path: ", er.path))
        },
      },
    },
...
fn json_schema_top_level() -> Result[Unit, Str] {
  ...
  # Should mention "$schema", "type":"object", and the field names.
  if str.contains(sch, "$schema")
    and str.contains(sch, "\"type\":\"object\"")
    and str.contains(sch, "\"name\":")
    and str.contains(sch, "\"age\":")
    and str.contains(sch, "\"required\":")
  { Ok(()) }
  else { Err("missing expected json-schema fields") }
}

fn json_schema_required_field_listed() -> Result[Unit, Str] {
  let sch := jv.stringify(s.to_json_schema(user_schema()))
  # `name` is required so it must appear in the required array.
  if str.contains(sch, "\"required\":[\"name\",\"age\"]") { Ok(()) }
  else { Err(str.concat("required wrong: ", sch)) }
}

fn json_schema_constraint_emitted() -> Result[Unit, Str] {
  let sch := jv.stringify(s.to_json_schema(user_schema()))
  # `name` carries `minLength: 1, maxLength: 80`.
  if str.contains(sch, "\"minLength\":1") and str.contains(sch, "\"maxLength\":80") {
    Ok(())
  } else { Err("constraint missing") }
}

fn json_schema_email_format() -> Result[Unit, Str] {
  let sch_obj := s.to_json_schema({
    title: "X", description: "",
    fields: [s.required_str("email", [StrEmail])],
  })
  let sch := jv.stringify(sch_obj)
  if str.contains(sch, "\"format\":\"email\"") { Ok(()) } else {
...
fn openapi_no_dollar_schema() -> Result[Unit, Str] {
  # The OpenAPI variant drops the `$schema` field (it lives at the
  # document root, not the component schema).
  let sch := jv.stringify(s.to_openapi_schema(user_schema()))
  if not (str.contains(sch, "$schema")) { Ok(()) } else {
...
fn suite() -> List[Result[Unit, Str]] {
  [
    validate_happy(),
    validate_missing_required(),
    validate_type_mismatch(),
    validate_optional_absent_ok(),
    validate_constraint_failure(),
    nested_path_in_error(),
    array_element_error_indexed(),
    array_shape_error(),
    json_schema_top_level(),
    json_schema_required_field_listed(),
    json_schema_constraint_emitted(),
    json_schema_email_format(),
    openapi_no_dollar_schema(),
  ]
}
