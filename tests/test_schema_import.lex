...
fn original() -> s.ModelSchema {
  { title: "Original", description: "", fields: [s.required_str("a", [StrMinLen(1), StrMaxLen(10)]), s.required_int("b", [IntInRange(0, 100)]), s.optional(s.required_str("c", [StrEmail]))] }
}
...
fn round_trip_preserves_required() -> Result[Unit, Str] {
  ...
  match si.from_str(text) {
    Err(_) => Err("import failed"),
    Ok(imported) => {
      let c_required := list.fold(imported.fields, true, fn (acc :: Bool, f :: s.Field) -> Bool {
        if f.name == "c" {
          f.required
        } else {
          acc
...
fn parses_minimal_schema() -> Result[Unit, Str] {
  match si.from_str("{\"type\":\"object\",\"properties\":{\"x\":{\"type\":\"string\"}},\"required\":[\"x\"]}") {
    Ok(m) => if list.len(m.fields) == 1 {
      Ok(())
    } else {
      Err("wrong count")
    },
...
fn parses_int_min_max() -> Result[Unit, Str] {
  match si.from_str("{\"type\":\"object\",\"properties\":{\"n\":{\"type\":\"integer\",\"minimum\":0,\"maximum\":99}},\"required\":[\"n\"]}") {
    Err(_) => Err("parse failed"),
    Ok(m) => match list.head(m.fields) {
      None => Err("no fields"),
      Some(f) => match f.kind {
        KInt(checks) => if list.len(checks) == 2 {
          Ok(())
        } else {
...
fn parses_email_format() -> Result[Unit, Str] {
  match si.from_str("{\"type\":\"object\",\"properties\":{\"e\":{\"type\":\"string\",\"format\":\"email\"}},\"required\":[\"e\"]}") {
    Err(_) => Err("parse failed"),
    Ok(m) => match list.head(m.fields) {
      None => Err("no fields"),
      Some(f) => match f.kind {
        KStr(checks) => {
          let has_email := list.fold(checks, false, fn (acc :: Bool, chk :: c.StrCheck) -> Bool {
            acc or match chk {
              StrEmail => true,
              _ => false,
            }
...
fn suite() -> List[Result[Unit, Str]] {
  [round_trip_preserves_fields(), round_trip_preserves_required(), parses_minimal_schema(), parses_int_min_max(), parses_email_format(), imported_schema_accepts_valid(), imported_schema_rejects_invalid()]
}
