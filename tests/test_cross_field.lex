...
fn accumulates_both() -> Result[Unit, Str] {
  # a != b AND a <= 0 — two failures from one record.
  match cm.cross_check(pair(0, 1), [equal_check, positive_check]) {
    Ok(_)   => Err("expected Err"),
    Err(es) => if list.len(es) == 2 { Ok(()) } else { Err("expected 2") },
  }
}
...
fn require_passes() -> Result[Unit, Str] {
  match cm.require(pair(1, 1),
                    fn (p :: Pair) -> Bool { p.a == p.b },
...
fn require_fails() -> Result[Unit, Str] {
  match cm.require(pair(1, 2),
                    fn (p :: Pair) -> Bool { p.a == p.b },
                    "a", "mismatch", "values must agree") {
    Ok(_)   => Err("expected Err"),
    Err(es) => match list.head(es) {
      None     => Err("empty"),
      Some(er) => if er.code == "mismatch" { Ok(()) } else {
...
fn make_pair(a :: Int, b :: Int) -> Pair { { a: a, b: b } }
