...
fn check_str_ok() -> Result[Unit, Str] {
  match f.check_str("name", "abc", [StrMinLen(1)]) {
    Ok(s)   => if s == "abc" { Ok(()) } else { Err("value not returned unchanged") },
...
fn check_str_accumulates_failures() -> Result[Unit, Str] {
  ...
  match r {
    ...
    Err(es) => if list.len(es) == 2 {
      ...
      Err(str.concat("expected 2 errors, got len=",
                     int_to_str_helper(list.len(es))))
    },
  }
}
...
fn check_str_path_propagates() -> Result[Unit, Str] {
  ...
  match r {
    ...
    Err(es)  => match list.head(es) {
      ...
      Some(err)  => if err.path == "user.email" { Ok(()) } else {
...
fn check_int_ok() -> Result[Unit, Str] {
  match f.check_int("age", 25, [IntInRange(0, 130)]) {
    Ok(n)  => if n == 25 { Ok(()) } else { Err("value changed") },
    Err(_) => Err("expected Ok"),
  }
}
...
fn is_lowercase(s :: Str) -> Option[Str] {
  if str.to_lower(s) == s { None } else { Some("must be all lowercase") }
}

fn validate_custom_pass() -> Result[Unit, Str] {
  match f.validate("tag", "abc", "lowercase", is_lowercase) {
    Ok(s)  => if s == "abc" { Ok(()) } else { Err("changed value") },
    Err(_) => Err("expected Ok"),
  }
}
...
fn validate_custom_fail() -> Result[Unit, Str] {
  match f.validate("tag", "ABC", "lowercase", is_lowercase) {
    ...
    Err(es)  => match list.head(es) {
      None       => Err("empty"),
      Some(er)   => if er.code == "lowercase" { Ok(()) } else {
        Err(str.concat("wrong code: ", er.code))
      },
    },
  }
...
fn suite() -> List[Result[Unit, Str]] {
  [
    check_str_ok(),
    check_str_accumulates_failures(),
    check_str_path_propagates(),
    check_int_ok(),
    check_int_below_min(),
    check_float_finite_passes(),
    validate_custom_pass(),
    validate_custom_fail(),
  ]
}
...
fn int_to_str_helper(n :: Int) -> Str { int.to_str(n) }
