...
fn chained() -> Result[Unit, Str] {
  ...
  let out := m.apply(p, [Rename({ from: "firstName", to: "first_name" }), Rename({ from: "lastName", to: "last_name" }), CoerceStrToInt("age"), NestInto({ name: "name", fields: ["first_name", "last_name"] })])
  match jv.get_field(out, "age") {
...
fn compat_identical_schemas() -> Result[Unit, Str] {
  let sch := { title: "X", description: "", fields: [s.required_str("a", [])] }
...
fn compat_added_required_breaks() -> Result[Unit, Str] {
  let old := { title: "X", description: "", fields: [s.required_str("a", [])] }
  let new := { title: "X", description: "", fields: [s.required_str("a", []), s.required_str("b", [])] }
  match m.is_backward_compatible(old, new) {
    Ok(_) => Err("should be incompatible"),
    Err(is) => if list.len(is) == 1 {
      Ok(())
    } else {
      Err("expected 1 issue")
    },
  }
}
...
fn compat_added_optional_ok() -> Result[Unit, Str] {
  let old := { title: "X", description: "", fields: [s.required_str("a", [])] }
  let new := { title: "X", description: "", fields: [s.required_str("a", []), s.optional(s.required_str("b", []))] }
  match m.is_backward_compatible(old, new) {
    Ok(_) => Ok(()),
...
fn compat_type_changed_breaks() -> Result[Unit, Str] {
  let old := { title: "X", description: "", fields: [s.required_str("a", [])] }
  let new := { title: "X", description: "", fields: [s.required_int("a", [])] }
  match m.is_backward_compatible(old, new) {
    Ok(_) => Err("should be incompatible"),
    Err(_) => Ok(()),
  }
...
fn compat_field_became_required_breaks() -> Result[Unit, Str] {
  let old := { title: "X", description: "", fields: [s.optional(s.required_str("a", []))] }
  let new := { title: "X", description: "", fields: [s.required_str("a", [])] }
  match m.is_backward_compatible(old, new) {
...
fn suite() -> List[Result[Unit, Str]] {
  [rename_works(), drop_removes(), add_when_absent(), add_skips_when_present(), coerce_int(), coerce_bad_drops(), chained(), compat_identical_schemas(), compat_added_required_breaks(), compat_added_optional_ok(), compat_type_changed_breaks(), compat_field_became_required_breaks()]
}
