...
fn every_category_100pct() -> Result[Unit, Str] {
  ...
  if bad == 0 { Ok(()) } else { Err("a category had escapes") }
}
...
fn format_includes_every_category() -> Result[Unit, Str] {
  ...
  if str.contains(s, "parse_failures:")
    and str.contains(s, "type_mismatches:")
    and str.contains(s, "missing_required:")
    and str.contains(s, "constraint_failures:")
    and str.contains(s, "deep_nesting:")
  { Ok(()) } else { Err("missing categories in format") }
}
...
fn suite() -> List[Result[Unit, Str]] {
  [
    zero_escapes(),
    every_category_100pct(),
    format_includes_every_category(),
    corpus_non_empty(),
  ]
}
