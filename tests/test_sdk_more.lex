...
fn zod_has_import() -> Result[Unit, Str] {
  ...
  if str.contains(out, "import { z } from \"zod\";") {
    Ok(())
  } else {
    Err("no zod import")
  }
...
fn zod_object_call() -> Result[Unit, Str] {
  let out := sdk.to_zod(user_schema())
  if str.contains(out, "export const User = z.object({") {
    Ok(())
  } else {
    Err("no z.object")
  }
...
fn zod_int_range() -> Result[Unit, Str] {
  ...
  if str.contains(out, "z.number().int()") and str.contains(out, ".min(13).max(130)") {
    ...
    Err(str.concat("missing int chain: ", out))
  }
}
...
fn rust_has_serde_import() -> Result[Unit, Str] {
  ...
  if str.contains(out, "use serde::{Deserialize, Serialize};") {
    Ok(())
  } else {
    Err("no serde import")
  }
...
fn rust_struct_def() -> Result[Unit, Str] {
  let out := sdk.to_rust_struct(user_schema())
  if str.contains(out, "pub struct User {") {
    Ok(())
  } else {
    Err("no struct def")
  }
...
fn rust_derives() -> Result[Unit, Str] {
  let out := sdk.to_rust_struct(user_schema())
  if str.contains(out, "#[derive(Debug, Clone, Serialize, Deserialize)]") {
    Ok(())
  } else {
    Err("no derives")
  }
...
fn rust_optional_uses_Option() -> Result[Unit, Str] {
  ...
  if str.contains(out, "Option<String>") and str.contains(out, "#[serde(default)]") {
    ...
    Err("optional not wrapped")
  }
}

fn rust_types_correct() -> Result[Unit, Str] {
  let out := sdk.to_rust_struct(user_schema())
  if str.contains(out, "pub name: String,") and str.contains(out, "pub age: i64,") {
    Ok(())
  } else {
    Err("wrong field types")
  }
}

# ---- Suite --------------------------------------------------------
fn suite() -> List[Result[Unit, Str]] {
  [zod_has_import(), zod_object_call(), zod_email_chain(), zod_int_range(), zod_optional_field(), rust_has_serde_import(), rust_struct_def(), rust_derives(), rust_optional_uses_Option(), rust_types_correct()]
}
