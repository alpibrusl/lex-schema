...
fn ts_has_interface_keyword() -> Result[Unit, Str] {
  ...
  if str.contains(out, "export interface User") { Ok(()) } else {
...
fn ts_optional_field_marked() -> Result[Unit, Str] {
  let out := sdk.to_typescript(simple_schema())
  if str.contains(out, "nickname?:") { Ok(()) } else {
...
fn ts_required_field_not_marked() -> Result[Unit, Str] {
  let out := sdk.to_typescript(simple_schema())
  if str.contains(out, "name: string") { Ok(()) } else {
...
fn ts_constraint_hint_in_comment() -> Result[Unit, Str] {
  let out := sdk.to_typescript(simple_schema())
  if str.contains(out, "minLength: 1") { Ok(()) } else {
...
fn ts_enum_becomes_union() -> Result[Unit, Str] {
  ...
  if str.contains(out, "\"free\" | \"pro\" | \"enterprise\"") { Ok(()) } else {
...
fn ts_nested_emitted() -> Result[Unit, Str] {
  ...
  # Both the outer Order and the nested Buyer should appear.
  if str.contains(out, "interface Order")
    and str.contains(out, "interface Buyer")
...
fn py_has_basemodel() -> Result[Unit, Str] {
  ...
  if str.contains(out, "class User(BaseModel)") { Ok(()) } else {
    Err("no class User")
  }
}
...
fn py_imports_present() -> Result[Unit, Str] {
  let out := sdk.to_python(simple_schema())
  if str.contains(out, "from pydantic import BaseModel, Field") { Ok(()) } else {
...
fn py_optional_uses_Optional() -> Result[Unit, Str] {
  let out := sdk.to_python(simple_schema())
  if str.contains(out, "nickname: Optional[str]") { Ok(()) } else {
...
fn py_constraint_args() -> Result[Unit, Str] {
  let out := sdk.to_python(simple_schema())
  if str.contains(out, "ge=13") and str.contains(out, "le=130") { Ok(()) } else {
    Err("missing ge/le")
  }
}
...
fn py_enum_uses_Literal() -> Result[Unit, Str] {
  ...
  if str.contains(out, "Literal[\"free\", \"pro\", \"enterprise\"]") { Ok(()) } else {
...
fn find_at(haystack :: Str, needle :: Str, i :: Int) -> Option[Int] {
  ...
  if i + nl > hl { None }
    ...
    if str.slice(haystack, i, i + nl) == needle { Some(i) }
    else { find_at(haystack, needle, i + 1) }
  }
}
...
fn suite() -> List[Result[Unit, Str]] {
  [
    ts_has_interface_keyword(),
    ts_optional_field_marked(),
    ts_required_field_not_marked(),
    ts_constraint_hint_in_comment(),
    ts_enum_becomes_union(),
    ts_nested_emitted(),
    py_has_basemodel(),
    py_imports_present(),
    py_optional_uses_Optional(),
    py_constraint_args(),
    py_enum_uses_Literal(),
    py_nested_class_above(),
  ]
}
