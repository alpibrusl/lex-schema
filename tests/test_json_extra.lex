...
fn get_path_missing_intermediate() -> Result[Unit, Str] {
  match jv.parse("{\"a\":{\"b\":1}}") {
    ...
    Ok(j)  => match jv.get_path(j, "a.b.c.d") {
      None    => Ok(()),     # nav stops at the int leaf
  ...
  match jv.parse("{\"user\":{}}") {
    ...
    Ok(j)  => match jv.j_str_at(j, "user.email", []) {
      ...
      Err(es) => match list.head(es) {
        None     => Err("empty"),
        Some(er) => if er.path == "user.email" { Ok(()) } else {
...
fn stringify_primitives() -> Result[Unit, Str] {
  if jv.stringify(JNull) == "null"
    and jv.stringify(JBool(true))  == "true"
    and jv.stringify(JBool(false)) == "false"
    and jv.stringify(JInt(42))     == "42"
    and jv.stringify(JStr("ok"))   == "\"ok\""
  { Ok(()) }
  else { Err("primitive stringify wrong") }
}
  ...
  match jv.parse(original) {
    ...
    Ok(j)  => {
      ...
      match jv.parse(serialized) {
        ...
        Ok(j2) => if jv.stringify(j2) == serialized { Ok(()) }
                  else { Err(str.concat("not stable: ", serialized)) },
      }
    },
  }
}
...
fn stringify_escapes() -> Result[Unit, Str] {
  # String with newline + quote should escape both.
...
fn stringify_pretty_indents() -> Result[Unit, Str] {
  match jv.parse("{\"a\":1}") {
    ...
    Ok(j)  => {
      ...
      # Should contain a newline and two-space indent.
...
fn suite() -> List[Result[Unit, Str]] {
  [
    get_path_one_segment(),
    get_path_nested(),
    get_path_missing_intermediate(),
    get_path_non_object_intermediate(),
    j_str_at_finds_nested(),
    j_str_at_path_in_error(),
    j_optional_str_at_absent(),
    stringify_primitives(),
    stringify_round_trip(),
    stringify_escapes(),
    stringify_pretty_empty(),
    stringify_pretty_indents(),
  ]
}
