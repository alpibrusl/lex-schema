...
fn gen_str_respects_bounds() -> Result[Unit, Str] {
  let schema := {
    title: "T", description: "",
    fields: [s.required_str("x", [StrMinLen(5), StrMaxLen(10)])],
  }
  ...
  match jv.get_field(v, "x") {
    Some(JStr(s)) => {
      let len := str.len(s)
      if len >= 5 and len <= 10 { Ok(()) } else { Err("out of range") }
    },
    _ => Err("no x field"),
  }
}
...
fn gen_int_respects_bounds() -> Result[Unit, Str] {
  let schema := {
    title: "T", description: "",
    fields: [s.required_int("n", [IntInRange(10, 20)])],
  }
  let g := p.generate(schema, random.seed(2))
  let v := match g { (v1, _) => v1 }
  match jv.get_field(v, "n") {
    Some(JInt(n)) => if n >= 10 and n <= 20 { Ok(()) } else { Err("out of range") },
    _ => Err("no n field"),
  }
}
...
fn gen_email_format() -> Result[Unit, Str] {
  ...
  match jv.get_field(v, "email") {
    Some(JStr(s)) => if str.contains(s, "@") { Ok(()) } else {
...
fn gen_uuid_format() -> Result[Unit, Str] {
  let schema := {
    title: "T", description: "",
    fields: [s.required_str("id", [StrUuid])],
  }
  let g := p.generate(schema, random.seed(4))
  let v := match g { (v1, _) => v1 }
  match jv.get_field(v, "id") {
    Some(JStr(s)) => if str.len(s) == 36 { Ok(()) } else {
...
fn gen_one_of_picks_from_set() -> Result[Unit, Str] {
  let schema := {
    title: "T", description: "",
    fields: [s.required_str("color", [StrOneOf(["red", "green", "blue"])])],
  }
...
fn deterministic_per_seed() -> Result[Unit, Str] {
  let schema := {
    title: "T", description: "",
    fields: [
      s.required_str("a", [StrMinLen(3), StrMaxLen(5)]),
      s.required_int("n", [IntInRange(0, 100)]),
    ],
  }
  let g1 := p.generate(schema, random.seed(42))
  let g2 := p.generate(schema, random.seed(42))
  let v1 := match g1 { (v, _) => v }
  let v2 := match g2 { (v, _) => v }
  if jv.stringify(v1) == jv.stringify(v2) { Ok(()) } else {
...
fn round_trip_small_schema() -> Result[Unit, Str] {
  let schema := {
    ...
    fields: [
      s.required_str("name", [StrMinLen(1), StrMaxLen(20)]),
      s.required_int("age",  [IntInRange(0, 120)]),
      s.required_bool("ok"),
    ],
  }
...
fn round_trip_with_array() -> Result[Unit, Str] {
  let schema := {
    ...
    fields: [
      s.required_array("items", KStr([StrMinLen(1), StrMaxLen(5)]),
                        [ListMinLen(1), ListMaxLen(4)]),
    ],
  }
...
fn suite() -> List[Result[Unit, Str]] {
  [
    gen_str_respects_bounds(),
    gen_int_respects_bounds(),
    gen_email_format(),
    gen_uuid_format(),
    gen_one_of_picks_from_set(),
    deterministic_per_seed(),
    round_trip_small_schema(),
    round_trip_with_array(),
  ]
}
