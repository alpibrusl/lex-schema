...
fn one_text_parses() -> Result[Unit, Str] {
  match form.decode_multipart(body_one_text(), b()) {
    ...
    Ok(parts) => if list.len(parts) == 1 { Ok(()) }
                 else { Err("wrong part count") },
  }
}
...
fn one_text_body() -> Result[Unit, Str] {
  match form.decode_multipart(body_one_text(), b()) {
    ...
    Ok(parts) => match list.head(parts) {
      None    => Err("no head"),
      Some(p) => if p.body == "hello" { Ok(()) }
                 else { Err(str.concat("body was: ", p.body)) },
    },
  }
}
...
fn one_text_name() -> Result[Unit, Str] {
  match form.decode_multipart(body_one_text(), b()) {
    ...
    Ok(parts) => match list.head(parts) {
      None    => Err("no head"),
      Some(p) => if p.name == "field" { Ok(()) }
                 else { Err(str.concat("name was: ", p.name)) },
    },
  }
}
...
fn one_text_default_ct() -> Result[Unit, Str] {
  match form.decode_multipart(body_one_text(), b()) {
    ...
    Ok(parts) => match list.head(parts) {
      None    => Err("no head"),
      Some(p) => if p.content_type == "text/plain" { Ok(()) }
                 else { Err(str.concat("ct: ", p.content_type)) },
    },
  }
}
  ...
  match form.decode_multipart(body_file_upload(), b()) {
    ...
    Ok(parts) => match list.head(parts) {
      ...
      Some(p) => match p.filename {
        ...
        Some(f) => if f == "notes.txt" { Ok(()) }
                   else { Err(str.concat("filename was: ", f)) },
      },
    },
  }
}
...
fn file_content_type() -> Result[Unit, Str] {
  match form.decode_multipart(body_file_upload(), b()) {
    ...
    Ok(parts) => match list.head(parts) {
      None    => Err("no head"),
      Some(p) => if p.content_type == "text/markdown" { Ok(()) }
                 else { Err(str.concat("ct: ", p.content_type)) },
    },
  }
}
...
fn file_body_preserved() -> Result[Unit, Str] {
  match form.decode_multipart(body_file_upload(), b()) {
    ...
    Ok(parts) => match list.head(parts) {
      None    => Err("no head"),
      Some(p) => if str.contains(p.body, "# title")
                    and str.contains(p.body, "line2") { Ok(()) }
                 else { Err(str.concat("body: ", p.body)) },
    },
  }
}
...
fn mixed_count() -> Result[Unit, Str] {
  match form.decode_multipart(body_mixed(), b()) {
    Err(_)    => Err("rejected"),
    Ok(parts) => if list.len(parts) == 3 { Ok(()) }
                 else { Err("wrong count") },
  }
}
...
fn mixed_ordering() -> Result[Unit, Str] {
  match form.decode_multipart(body_mixed(), b()) {
    Err(_)    => Err("rejected"),
    Ok(parts) => {
      let names := list.map(parts,
        fn (p :: form.MultipartPart) -> Str { p.name })
      let joined := str.join(names, ",")
      if joined == "user,avatar,remember" { Ok(()) }
      else { Err(str.concat("got: ", joined)) }
    },
  }
}
...
fn mixed_avatar_body() -> Result[Unit, Str] {
  match form.decode_multipart(body_mixed(), b()) {
    ...
    Ok(parts) => match list.head(list.tail(parts)) {
      ...
      Some(p) => if p.body == "PNGDATA"
                    and p.content_type == "image/png" { Ok(()) }
                 else { Err(str.concat("body: ", p.body)) },
    },
  }
}
...
fn blank_line_in_body() -> Result[Unit, Str] {
  match form.decode_multipart(body_with_blank_line(), b()) {
    ...
    Ok(parts) => match list.head(parts) {
      None    => Err("no head"),
      Some(p) => if str.contains(p.body, "paragraph one")
                    and str.contains(p.body, "paragraph two") { Ok(()) }
                 else { Err(str.concat("body: ", p.body)) },
    },
  }
}
...
fn dispatch_multipart_flattens_to_map() -> Result[Unit, Str] {
  ...
  match form.decode_body(body_mixed(), ct) {
    Err(_) => Err("rejected"),
    Ok(m)  => if map.size(m) == 3 { Ok(()) } else { Err("map size") },
  }
}
...
fn suite() -> List[Result[Unit, Str]] {
  [
    ...
    wrong_boundary(),
    empty_boundary(),
    dispatch_multipart_ok(),
    dispatch_multipart_quoted_boundary(),
    dispatch_multipart_missing_boundary(),
    dispatch_multipart_flattens_to_map(),
  ]
}
