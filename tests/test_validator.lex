...
fn serialize_valid_input() -> Result[Unit, Str] {
  ...
  match v.serialize(val, input) {
    Ok(s)   => if str.contains(s, "bob") { Ok(()) }
               else { Err(str.concat("unexpected output: ", s)) },
    Err(_)  => Err("expected Ok"),
  }
}
...
fn serialize_strips_extra_fields() -> Result[Unit, Str] {
  ...
  match v.serialize(val, input) {
    Ok(s)   => if not (str.contains(s, "secret")) { Ok(()) }
               else { Err("extra field leaked into output") },
    Err(_)  => Err("expected Ok"),
  }
}
...
fn serialize_invalid_input_returns_err() -> Result[Unit, Str] {
  ...
  match v.serialize(val, input) {
    Ok(_)   => Err("expected Err"),
    Err(es) => if list.len(es) >= 1 { Ok(()) } else { Err("no errors") },
  }
}
...
fn serialize_strict_extra_field_rejected() -> Result[Unit, Str] {
  ...
  match v.serialize_strict(val, input) {
    ...
    Err(es) => match list.head(es) {
      ...
      Some(er) => if er.code == "extra_field" { Ok(()) }
                  else { Err(str.concat("wrong code: ", er.code)) },
    },
  }
}
...
fn serialize_strict_extra_field_path_is_key_name() -> Result[Unit, Str] {
  ...
  match v.serialize_strict(val, input) {
    ...
    Err(es) => match list.head(es) {
      None     => Err("empty"),
      Some(er) => if er.path == "role" { Ok(()) }
                  else { Err(str.concat("wrong path: ", er.path)) },
    },
  }
}
...
fn serialize_lossy_drops_extras() -> Result[Unit, Str] {
  ...
  if not (str.contains(out, "extra")) { Ok(()) }
  else { Err("extra field leaked") }
}
...
fn serialize_lossy_invalid_returns_empty_obj() -> Result[Unit, Str] {
  ...
  let out := v.serialize_lossy(val, input)
  if out == "{}" { Ok(()) }
  else { Err(str.concat("expected {}, got: ", out)) }
}
  ...
  match jv.get_path(resp, "200.content.application/json.schema") {
    ...
    Some(schema) => match jv.get_field(schema, "title") {
      ...
      Some(t) => match jv.as_str(t) {
        Some(s) => if s == "User" { Ok(()) } else { Err(str.concat("wrong title: ", s)) },
...
fn suite() -> List[Result[Unit, Str]] {
  [
    ...
    serialize_valid_input(),
    serialize_strips_extra_fields(),
    serialize_invalid_input_returns_err(),
    serialize_strict_clean_input_passes(),
    serialize_strict_extra_field_rejected(),
    serialize_strict_extra_field_path_is_key_name(),
    serialize_lossy_drops_extras(),
    serialize_lossy_invalid_returns_empty_obj(),
    openapi_response_has_200_key(),
    openapi_response_has_content_schema(),
    openapi_response_schema_has_title(),
  ]
}
