...
fn strict_rejects_one_extra() -> Result[Unit, Str] {
  ...
  match v.serialize_strict(val, json_with_extra()) {
    ...
    Err(es) => {
      let has := list.fold(es, false,
        fn (acc :: Bool, err :: e.Error) -> Bool {
          acc or (err.code == "unexpected" and err.path == "password_hash")
        })
      if has { Ok(()) } else { Err("unexpected error not surfaced") }
    },
  }
}
...
fn strict_reports_every_extra() -> Result[Unit, Str] {
  ...
  match v.serialize_strict(val, json_with_two_extras()) {
    Ok(_)   => Err("strict should not pass with extras"),
    Err(es) => {
      let unexpected := list.fold(es, 0,
        fn (acc :: Int, err :: e.Error) -> Int {
          if err.code == "unexpected" { acc + 1 } else { acc }
        })
      if unexpected == 2 { Ok(()) }
      else { Err("expected 2 unexpected errors") }
    },
  }
}
...
fn strict_rejects_non_object() -> Result[Unit, Str] {
  ...
  match v.serialize_strict(val, JStr("not an object")) {
    ...
    Err(es) =>
      if list.len(es) >= 1 { Ok(()) }
      else { Err("non-object surfaced no errors") },
  }
}
...
fn serialize_defaults_to_lossy() -> Result[Unit, Str] {
  ...
  match v.serialize(val, json_with_extra()) {
    Ok(out) =>
      if not (str.contains(out, "password_hash")) { Ok(()) }
      else { Err("default should drop extras") },
...
fn openapi_response_shape() -> Result[Unit, Str] {
  ...
  if str.contains(s, "description")
     and str.contains(s, "content")
     and str.contains(s, "application/json")
     and str.contains(s, "schema") { Ok(()) }
  else { Err(str.concat("openapi_response missing keys: ", s)) }
}

fn openapi_response_uses_title() -> Result[Unit, Str] {
  let val := v.make(schema())
  let s := jv.stringify(v.openapi_response(val))
  if str.contains(s, "User") { Ok(()) } else { Err("title not in description") }
}
...
fn suite() -> List[Result[Unit, Str]] {
  [
    ...
    strict_rejects_non_object(),
    serialize_defaults_to_lossy(),
    openapi_response_shape(),
    openapi_response_uses_title(),
  ]
}
